...
void main()
{
    ...
    // positionCopy.z += aRandomMove.z * uTime;
	// vec3 vel = (positionCopy - position) / 0.1;

    // vel = 0.95 * vel + 0.01 * snoiseVec3(position + vec3(uTime)) ;

    // positionCopy = 0.95 * positionCopy + snoiseVec3(position + vec3(uTime*0.05)) * 0.03;

    vec3 dir = position - positionCopy;
    float dist = length(dir);
    dir /= dist;


    vec3 inputForceDir = positionCopy - uMousePos;
    ...
    vec3 acc =  0.5 * dir * dist * aRandomMove;
